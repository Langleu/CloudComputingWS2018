...
# for vio v3
READWRITE="$(fio --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=random --filename=randomFile --bs=4M --iodepth=64 --size=1024M --readwrite=randrw -rwmixread=75 2>&1 | awk '/READ: bw=/ {printf $2 }')"
...
READ=$(echo ${READWRITE##*=})
TEMP=$(echo "${READ::-5}")
READ=$(awk "BEGIN {printf \"%.1f\", $TEMP*1.25*1000}")
echo $READ
